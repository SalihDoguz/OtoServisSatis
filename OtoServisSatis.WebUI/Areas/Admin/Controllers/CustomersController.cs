    ...
    public class CustomersController : Controller
    {
        ...
        public async Task<ActionResult> EditAsync(int id, Musteri musteri)
        {
            if(ModelState.IsValid)
            {
                try
                {
                    _service.Update(musteri); // Burada güncelleme işlemi yapmış olduk.
                    await _service.SaveAsync(); // burada veritabanına kayıt işelemi yapmış bulunmaktayız.
                    return RedirectToAction(nameof(Index));
                }
                catch
                {
                    ModelState.AddModelError("", "Müşteri güncellerken hata oluştu!");
                }
            }
            ViewBag.AracId = new SelectList( await _serviceArac.GetAllAsync(), "Id", "Model");
            return View(musteri);
